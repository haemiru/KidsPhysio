...
| **환경변수 설정 (로컬 `.env.local`)** | ✅ **재복구 완료** — Solapi 3개(API키/시크릿/발신번호) 제외 17개 채움. 상세 §11·§13 |
| **환경변수 설정 (Vercel `kids-physio`)** | ✅ **완료(2026-06-04)** — production+preview에 16키 등록. Solapi 3개만 공란. |
...
- [ ] **Solapi `API_KEY`/`API_SECRET`/`SENDER_PHONE` 재확보** — `.env.local` 유실로 복구 불가(§13). console.solapi.com에서 재확인/재발급 후 로컬+Vercel에 채울 것. (없어도 결제흐름 정상, 알림톡만 skip)
...
### 남은 일
1. Solapi 3개 값 재확보(위) → 로컬+Vercel 등록(카카오 템플릿 검수 승인 후 실발송).
