...
## 15. ★ 진행 중: 키즈피지오 전용 카카오 채널 분리 (2026-06-11 — 채널 승인·연동 완료, 새 템플릿 4종 검수 중)
...
- ⏳ **대기**: 새 템플릿 4종 카카오 검수(영업일 1~2일). **env는 아직 교체하지 않음** — 현행 짱샘책방 채널/템플릿(APPROVED·동작중)을 유지. 새 4종이 전부 승인되기 전에 교체하면 알림톡 발송이 막힘.
...
**★ 다시 돌아오면 곧바로 할 일**(1·2단계는 ⑤⑥에서 완료, 남은 건 검수 승인 후 교체뿐):
1. **검수 상태 확인**: `node scripts/solapi-new-channel-status.mjs` — 새 템플릿 4종이 전부 `APPROVED`면 자동으로 교체용 env 블록을 출력함. (반려면 사유 확인 후 본문 보강 → 재신청)
2. **승인되면 env 교체 + 재배포 + 재검증**:
   - `.env.local`의 `SOLAPI_PFID` + `SOLAPI_TPL_*` 4개를 위 ⑥ 표의 **새 값으로 교체**.
   - Vercel(prod/preview)에도 동일 5키 교체 → 재배포.
   - `node scripts/solapi-test.mjs`로 잔액·템플릿상태·실발송(`--send`) 재검증 → 발신자명이 **"키즈피지오"**로 도착하는지 확인.
   - 짱샘책방 채널/템플릿은 그대로 두면 됨(책방 프로젝트가 계속 사용).
