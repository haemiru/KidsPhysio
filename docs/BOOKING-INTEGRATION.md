...
| **환경변수 설정 (로컬 `.env.local`)** | ✅ **완료** — 결제·예약·알림톡(PFID/템플릿/관리자번호) 모두 채움. 상세 §11 |
| **환경변수 설정 (Vercel 배포용)** | ⏳ 미등록 — 배포 시 Vercel 프로젝트에 동일하게 넣어야 함 |
...
- [ ] **⭐ 최우선: `vercel dev`로 예약 흐름(신청→슬롯→결제) 실제 동작 확인.**
- [ ] **Vercel 배포 프로젝트에 환경변수 동일 등록**(배포 시).
