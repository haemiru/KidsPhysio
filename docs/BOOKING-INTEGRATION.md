...
| 솔라피 알림톡 템플릿 승인 | ⏳ **키즈피지오 전용 채널로 이전 — 새 4종 검수중**(2026-06-11). 짱샘책방 채널의 구 4종은 APPROVED였으나 발신자명이 "주식회사 짱샘에듀"라 키즈피지오 전용 채널(pfId `…819bHStn9Bcoyw`) 신설→ 동일본문 4종 재등록·검수요청(INSPECTING). env·Vercel·배포는 새 채널로 **이미 교체 완료**(§15 ⑥⑦). 승인되면 즉시 발송 가능. ⚠️ 잔액 0/포인트 287 — SMS 대체발송 불가. |
...
- ⏳ **대기**: 새 템플릿 4종 카카오 검수(영업일 1~2일). 승인되면 추가 작업 없이 바로 발송 가능(env·배포 이미 새 채널 기준).
...
**★ 다시 돌아오면 곧바로 할 일**(env 교체·배포는 ⑦에서 완료. 남은 건 검수 승인 확인뿐):
1. **검수 상태 확인**: `node scripts/solapi-new-channel-status.mjs` (또는 `solapi-test.mjs`) — 새 템플릿 4종이 전부 `APPROVED`로 바뀌었는지 확인.
   - **승인이면**: 추가 작업 없음. 오픈 전 `node scripts/solapi-test.mjs --send`로 실발송 1건 → 발신자명이 **"키즈피지오"**로 도착하는지만 확인.
   - **반려면**: 사유 확인 → 본문 보강 후 `PUT /kakao/v2/templates/{id}/inspection`으로 재검수(또는 솔라피 콘솔에서 재요청). templateId는 보통 유지되므로 env 교체 불필요.
2. ⚠️ **잔액 0/포인트 287** — 알림톡은 포인트 차감(가능), 단 SMS 대체발송은 잔액 0이라 불가. 오픈 전 솔라피 캐시 충전 검토.
